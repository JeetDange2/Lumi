...
export default function LeaderboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f11] text-white font-sans relative overflow-x-hidden pb-20 custom-scrollbar">
      ...
      <div className="fixed top-[-100px] left-1/2 -translate-x-1/2 w-[800px] h-[500px] rounded-full bg-fuchsia-700/10 blur-[150px] pointer-events-none" />

      {/* Navigation */}
      <nav className="relative z-10 px-6 md:px-12 py-6 flex flex-col md:flex-row gap-4 items-center justify-between">
        <button onClick={() => navigate("/profile")} className="group flex items-center gap-2 text-zinc-400 hover:text-white transition-colors w-full md:w-auto">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
            <input 
               ...
               className="bg-[#18181b] border border-white/[0.1] text-sm text-white rounded-full px-4 py-2 flex-1 md:w-56 focus:outline-none focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500 transition-all placeholder:text-zinc-600"
            />
            <button onClick={handleAddFriend} className="bg-fuchsia-500/20 text-fuchsia-400 hover:bg-fuchsia-500 hover:text-white px-4 py-2 rounded-full text-sm font-bold transition-all shadow-[0_0_15px_rgba(217,70,239,0.15)] flex items-center gap-1 shrink-0">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 mt-2 text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          {pendingRequests.length > 0 && (
             <div className="flex flex-col gap-3 max-w-xl mx-auto mb-10">
                 {pendingRequests.map(req => (
                     <div key={req.id} className="bg-[#18181b] border border-fuchsia-500/30 rounded-2xl p-4 flex items-center justify-between shadow-[0_0_25px_rgba(217,70,239,0.15)] animate-in slide-in-from-top-4 duration-500">
                         <div className="flex items-center gap-4">
                             <div className="w-12 h-12 rounded-full bg-gradient-to-br from-fuchsia-600 to-orange-500 text-white flex items-center justify-center font-bold text-xl shadow-inner shadow-white/20">
                                 {req.sender.charAt(0).toUpperCase()}
                             </div>
                             <div className="text-left">
                                 <h4 className="text-base font-bold text-white leading-tight">{req.sender}</h4>
          ...
          <div className="flex justify-center mb-16">
              <div className="bg-[#18181b] p-1.5 rounded-full border border-white/[0.1] flex relative shadow-2xl w-72 h-14">
                   ...
                   <button 
                      ...
                      className={`relative z-10 flex-1 rounded-full text-sm font-bold transition-colors duration-300 flex items-center justify-center gap-2 ${viewMode === 'global' ? 'text-white' : 'text-zinc-500 hover:text-white'}`}
                   >
                     🌍 Global Rank
                   </button>
                   <button 
                      onClick={() => setViewMode('friends')}
                      className={`relative z-10 flex-1 rounded-full text-sm font-bold transition-colors duration-300 flex items-center justify-center gap-2 ${viewMode === 'friends' ? 'text-white' : 'text-zinc-500 hover:text-white'}`}
                   >
                     🤝 Friends Only
                   </button>
              </div>
          ...
          {/* Top 3 Podium */}
          {top3.length > 0 && (
              <div className="flex justify-center items-end gap-2 md:gap-6 mb-16 h-64 border-b border-white/[0.05] pb-8">
                  {/* Rank 2 */}
                  {top3[1] && (
                  ...
                  {top3[0] && (
                    <div className="flex flex-col items-center w-32 md:w-40 animate-in fade-in zoom-in-50 duration-700 relative z-20 mx-2">
                        ...
                        <div className="w-full h-40 bg-gradient-to-t from-yellow-500/5 to-yellow-500/20 border-t border-l border-r border-yellow-500/30 rounded-t-lg flex flex-col items-center pt-3 relative overflow-hidden backdrop-blur-md">
                            ...
                            <span className="text-sm font-bold text-white mt-1 truncate w-28 text-center">{top3[0].name}</span>
          ...
          {leaderboard.length === 1 && viewMode === 'friends' && (
              <div className="py-12 border border-white/[0.05] rounded-3xl bg-white/[0.01]">
                  <div className="w-16 h-16 bg-white/[0.05] rounded-full mx-auto flex items-center justify-center mb-4">
          ...
          {/* List Remaining */}
          {rest.length > 0 && (
            <div className="bg-[#18181b]/80 backdrop-blur-xl border border-white/[0.05] rounded-3xl p-4 md:p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-10 duration-700 delay-300">
                <div className="space-y-3">
                    {rest.map((player, idx) => (
                        <div key={player.id} className={`flex items-center gap-4 p-4 rounded-xl transition-all ${player.isUser ? 'bg-fuchsia-500/10 border border-fuchsia-500/30 transform scale-[1.02] shadow-lg shadow-fuchsia-900/20' : 'bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.05]'}`}>
                            <span className="text-zinc-500 font-bold w-6 text-center">#{idx + 4}</span>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${player.isUser ? 'bg-fuchsia-600 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                                {player.avatar}
                            </div>
                            <span className={`font-semibold md:text-lg flex-1 text-left ${player.isUser ? 'text-white' : 'text-zinc-300'}`}>{player.name}</span>
